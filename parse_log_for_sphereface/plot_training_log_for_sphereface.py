...
def create_field_index():
    ...
    field_index = {train_key: {'Iters': 0, 'Seconds': 1, train_key + ' loss': 2,
                   ...
                   test_key: {'Iters': 0, 'Seconds': 1, test_key + ' accuracy': 2,
                              test_key + ' loss': 3, test_key + ' top5_accuracy': 5}}
    fields = set()
...
def get_field_indices(x_axis_field, y_axis_field):
    ...
    # print 'create_field_index()[0]: ', create_field_index()[0]
...
def load_data(data_file, field_idx0, field_idx1):
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if line[0] != '#':
                fields = line.split()
                if len(fields) < field_idx0 + 1 or len(fields) < field_idx1 + 1:
                    # break
                    continue
...
def get_legend_loc(chart_type):
    ...
    if y_axis.find('accuracy') != -1:
        pass
    if y_axis.find('loss') != -1 or y_axis.find('learning rate') != -1 or y_axis.find('lambda') != -1:
        loc = 'upper right'
    return loc
